...
import { extraction_external_sync_unit_check } from './index';
...
describe('extraction_external_sync_unit_check function', () => {
  ...
  it('should return success when external sync units extraction completes successfully', async () => {
    ...
    // Act
    const result = await extraction_external_sync_unit_check(mockEvents);
  ...
  it('should return false when spawn throws an error', async () => {
    ...
    // Assert
    expect(result.success).toBe(false);
    expect(result.message).toContain('Failed to execute external sync units extraction');
  ...
  it('should handle unexpected errors gracefully', async () => {
    ...
    // Assert
    expect(result.success).toBe(false);
    expect(result.message).toContain('Error testing external sync units extraction');
    expect(readInitialDomainMapping).not.toHaveBeenCalled();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error in extraction_external_sync_unit_check function:', mockError);
  });
});